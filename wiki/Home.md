# Margin Wiki

Margin is a local-first personal finance tracker. This Wiki is the long-form project reference; the repository README is the concise entry point and `project/` contains the working roadmap and task briefs.

## Wiki pages

- [Product Vision](Product-Vision.md)
- [Architecture](Architecture.md)
- [Privacy and Data](Privacy-and-Data.md)
- [Project Workflow](Project-Workflow.md)
- [Decision Log](Decision-Log.md)
- [Next Work and Idea Triage](Next-Work.md)

## Quick links

- [Repository README](../README.md)
- [Project context](../docs/PROJECT_CONTEXT.md)
- [v0.1.0 release notes](../docs/RELEASE-v0.1.0.md)
- [First epic](../project/epics/EPIC-001-foundation.md)
